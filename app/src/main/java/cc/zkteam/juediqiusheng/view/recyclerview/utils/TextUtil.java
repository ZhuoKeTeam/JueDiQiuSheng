...
public class TextUtil {
     ...
     * @return
     */
    public static boolean checkTBTitleValid(String title) {

        //data:屏蔽的是微信，微信会往剪切板里放东西
        if (TextUtil.isEmpty(title) || TextUtil.size(title) > 40 || title.startsWith("data:"))
            return false;

        try {

            boolean result = !title.matches("-?[0-9]+\\.?[0-9]*");//检测是否是纯数值
            if (result)
                result = !title.matches("[\\s\\S]*￥[0-9,a-z,A-Z]{5,20}￥[\\s\\S]*");//检测是否是淘口令
            return result;
    ...
    public static boolean contains(String origin, String containStr) {

        if (isEmpty(origin) || isEmpty(containStr))
            return false;
        else
            return origin.contains(containStr);
    }
     ...
     * @return
     */
    public static boolean isPatternCode(String text, String code) {

        if (TextUtil.isEmpty(text) || TextUtil.isEmpty(code))
            return false;
